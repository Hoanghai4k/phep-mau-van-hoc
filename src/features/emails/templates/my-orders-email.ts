...
export function buildMyOrdersEmailSubject(): string {
  return `Xem đơn hàng của bạn – ${siteConfig.name}`;
}
  ...
  return `
    ...
    <body>
      <div class="container">
        ...
        <p>Chào bạn,</p>
        <p>Bạn vừa yêu cầu truy cập danh sách đơn hàng của mình tại <strong>${siteConfig.name}</strong>. Hãy nhấn vào nút bên dưới để xác minh và xem đơn hàng của bạn:</p>
        ...
        <div class="footer">
          ...
          <p>Cảm ơn bạn đã đồng hành cùng ${siteConfig.name}!</p>
        </div>
      </div>
    </body>
    </html>
