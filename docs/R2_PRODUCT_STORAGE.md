...
2. Bucket name: `phepmauvanhoc-product-files`
...
3. Bucket scope: `phepmauvanhoc-product-files` only
4. TTL: No expiration (or set rotation policy)
5. Save the **Access Key ID** and **Secret Access Key**

### 3. Configure Bucket CORS

In the Cloudflare Dashboard → R2 → `phepmauvanhoc-product-files` → Settings → CORS:

```json
[
  {
    "AllowedOrigins": [
      "https://phepmauvanhoc.com",
      "https://www.phepmauvanhoc.com",
      "http://localhost:3000"
    ],
...
R2_BUCKET_NAME=phepmauvanhoc-product-files
