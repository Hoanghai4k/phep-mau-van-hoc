...
describe("My Orders Magic Link URL Generation", () => {
  ...
  it("should use SITE_URL when present (Production configuration)", async () => {
    process.env.SITE_URL = "https://www.phepmauvanhoc.com";
    
    await sendMyOrdersAccessEmail("user@example.com", "fake-token");
    
    const callArgs = mockSendEmail.mock.calls[0][0];
    expect(callArgs.html).toContain("https://www.phepmauvanhoc.com/orders/verify?token=fake-token");
  ...
  it("should use VERCEL_URL as fallback when SITE_URL is absent (Preview environment)", async () => {
    process.env.VERCEL_URL = "phep-mau-van-hoc-preview.vercel.app";
    
    await sendMyOrdersAccessEmail("user@example.com", "fake-token");
    
    const callArgs = mockSendEmail.mock.calls[0][0];
    expect(callArgs.html).toContain("https://phep-mau-van-hoc-preview.vercel.app/orders/verify?token=fake-token");
    expect(callArgs.html).not.toContain("localhost");
  });
  ...
  it("should strip trailing slashes from configured URLs", async () => {
    process.env.SITE_URL = "https://www.phepmauvanhoc.com/"; // trailing slash
